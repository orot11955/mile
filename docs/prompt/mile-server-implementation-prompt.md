...
아래 프롬프트는 `mile` 프로젝트의 `server` 영역을 구현하기 위해 다른 AI 개발 도구, 코드 생성 도구, 또는 개발 세션에 그대로 전달할 수 있는 요청서다.

---

## 1. 역할 부여

당신은 개인 생산성 관리 서비스 `mile`의 `server` 영역을 설계하고 구현하는 시니어 소프트웨어 엔지니어다.
...
```txt
대상: mile-server
권장 기술 스택: NestJS + TypeScript + Prisma + PostgreSQL + Redis/BullMQ + Docker Compose
```
...
아래 내용을 기준으로 `mile-server`를 구현하라.
...
먼저 이 계획서를 바탕으로 `mile-server`의 초기 프로젝트 구조와 MVP 구현 순서를 제안하라.
